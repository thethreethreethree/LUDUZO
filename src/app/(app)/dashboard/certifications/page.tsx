...
function expiryState(expires_on: string | null): { label: string; cls: string } {
  if (!expires_on) return { label: "no expiry", cls: "text-zinc-500" };
  const days = Math.floor((Date.parse(expires_on) - new Date().getTime()) / (24 * 3600 * 1000));
  if (days < 0) return { label: `expired ${-days}d ago`, cls: "text-red-500 font-medium" };
  if (days <= 30) return { label: `expires in ${days}d`, cls: "text-gold font-medium" };
  return { label: `valid · ${expires_on}`, cls: "text-zinc-500" };
}
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 p-8">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight">Certifications</h1>
      ...
      {orgs.length === 0 ? (
        ...
        <form action={addCertification} className="flex flex-col gap-3 rounded-md border border-onyx bg-onyx p-5">
          ...
          <div className="flex gap-3">
            ...
            <label className="flex min-w-0 flex-1 flex-col gap-1 text-xs text-zinc-500">Certification
              <input name="name" required placeholder="e.g. CPR / L2 PT" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
            </label>
          </div>
          <div className="flex gap-3">
            <label className="flex min-w-0 flex-1 flex-col gap-1 text-xs text-zinc-500">Issuer
              <input name="issuer" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
            </label>
            <label className="flex min-w-0 flex-1 flex-col gap-1 text-xs text-zinc-500">Issued
              <input name="issued_on" type="date" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
            </label>
            <label className="flex min-w-0 flex-1 flex-col gap-1 text-xs text-zinc-500">Expires
              <input name="expires_on" type="date" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
            </label>
          </div>
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-sm font-medium text-zinc-500">On file</h2>
        {certs.length === 0 ? (
          <p className="text-sm text-zinc-500">No certifications recorded.</p>
        ) : (
