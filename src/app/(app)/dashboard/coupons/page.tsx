    ...
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 p-8">
      ...
      {orgs.length > 0 ? (
        <form action={createCoupon} className="flex flex-wrap items-end gap-2 rounded-md border border-onyx bg-onyx p-4">
          ...
          <input name="discount_value" type="number" min="0" step="0.01" required placeholder="Value" className="w-24 rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
          <button className="rounded-md bg-gold px-4 py-2 text-sm font-medium text-black hover:opacity-90">
            Create
          </button>
        </form>
      ) : (
        <p className="text-sm text-zinc-500">You need a manager+ role to create coupons.</p>
      )}
    </main>
  );
}
