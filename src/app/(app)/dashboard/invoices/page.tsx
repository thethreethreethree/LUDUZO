    ...
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 p-8">
      <header className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 text-h1 text-bone">Invoices</h1>
        </div>
        <a
        ...
        <section className="grid gap-3 sm:grid-cols-2">
          ...
          <form action={suspendOverdue} className="flex flex-col justify-between gap-2 rounded-md border border-onyx bg-onyx p-4">
            <div>
              ...
              <p className="mt-1 text-xs text-ash">Freeze active members who have a past-due invoice.</p>
            </div>
            <OrgPicker orgs={orgs} />
        ...
        <ul className="flex flex-col divide-y divide-onyx rounded-md border border-onyx dark:divide-onyx dark:border-onyx">
          {invoices.map((inv) => (
            <li key={inv.id} className="flex items-center justify-between px-4 py-3">
              ...
              {inv.status === "paid" ? (
                <span className="text-xs text-win">paid</span>
              ) : inv.status === "void" ? (
                <span className="text-xs text-ash">void</span>
              ) : (
                <div className="flex items-center gap-2">
                  ...
                  <form action={voidInvoice}>
                    <input type="hidden" name="id" value={inv.id} />
                    <button className="rounded-md border border-iron px-3 py-1 text-xs font-medium text-ash hover:bg-onyx-2 hover:bg-onyx-2">
                      Void
                    </button>
                  </form>
                </div>
      ...
      {orgs.length > 0 ? (
        ...
        <p className="text-sm text-ash">You need a staff role in a gym to create invoices.</p>
      )}
    </main>
  );
}
