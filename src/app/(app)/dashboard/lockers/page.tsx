  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 p-8">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight">Locker rentals</h1>
        <p className="text-sm text-zinc-500">{active.length} active · {formatMoney(mrr)}/mo.</p>
      </div>
      ...
      {orgs.length > 0 ? (
        <form action={rentLocker} className="flex flex-wrap items-end gap-3 rounded-md border border-onyx bg-onyx p-5">
          <OrgPicker orgs={orgs} />
          <input name="locker_label" required placeholder="Locker #" className="w-24 rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
          <select name="member_id" className="min-w-0 flex-1 rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900">
            <option value="">— member —</option>
            {members.map((m) => (<option key={m.id} value={m.id}>{m.first_name} {m.last_name}</option>))}
          </select>
          <input name="monthly_fee" type="number" step="0.01" min="0" placeholder="$/mo" className="w-24 rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
          <button className="rounded-md bg-gold px-4 py-2 text-sm font-medium text-black hover:opacity-90">Rent</button>
        </form>
      ) : null}

      <section className="flex flex-col gap-2">
        <h2 className="text-sm font-medium text-zinc-500">Lockers</h2>
        {rentals.length === 0 ? (
          <p className="text-sm text-zinc-500">No locker rentals.</p>
        ) : (
            ...
            {rentals.map((r) => (
              <li key={r.id} className={`flex items-center justify-between gap-3 px-4 py-3 text-sm ${r.status === "active" ? "" : "opacity-50"}`}>
                <div className="flex min-w-0 flex-col">
                  ...
                  <span className="text-xs text-zinc-500">{r.monthly_fee_cents ? `${formatMoney(r.monthly_fee_cents)}/mo · ` : ""}{r.status}</span>
                </div>
                {r.status === "active" ? (
