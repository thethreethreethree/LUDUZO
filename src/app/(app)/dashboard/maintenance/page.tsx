  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 p-8">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight">Maintenance &amp; suppliers</h1>
        <p className="text-sm text-zinc-500">{down.length > 0 ? `${down.length} item(s) currently down. ` : ""}Keep equipment serviced.</p>
      </div>
      ...
      {orgs.length > 0 ? (
        <div className="grid gap-4 sm:grid-cols-2">
          <form action={logMaintenance} className="flex flex-col gap-2 rounded-md border border-onyx bg-onyx p-4">
            ...
            <OrgPicker orgs={orgs} />
            <select name="equipment_id" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900">
              <option value="">— general —</option>
              {equipment.map((e) => (<option key={e.id} value={e.id}>{e.name}</option>))}
            </select>
            <div className="flex gap-2">
              <select name="kind" className="w-28 rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900">
                <option value="service">service</option><option value="repair">repair</option><option value="inspection">inspection</option>
              </select>
              <input name="scheduled_for" type="date" className="min-w-0 flex-1 rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
            </div>
            <label className="flex items-center gap-2 text-xs text-zinc-400"><input type="checkbox" name="down" /> mark out of service</label>
            <input name="notes" placeholder="Notes" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
          ...
          <form action={addSupplier} className="flex flex-col gap-2 rounded-md border border-onyx bg-onyx p-4">
            ...
            <OrgPicker orgs={orgs} />
            <input name="name" required placeholder="Supplier name" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
            <input name="contact_name" placeholder="Contact" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
            <div className="flex gap-2">
              ...
              <input name="phone" placeholder="Phone" className="w-28 rounded-md border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
            </div>
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-sm font-medium text-zinc-500">Maintenance log</h2>
        {records.length === 0 ? (
              ...
              <li key={r.id} className="flex items-center justify-between gap-3 px-4 py-3 text-sm">
                <div className="flex min-w-0 flex-col">
                  <span className="font-medium">{r.equipment?.name ?? "General"} · {r.kind}
                    {r.down && !r.completed_at ? <span className="ml-2 text-red-500">DOWN</span> : null}
                  </span>
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-sm font-medium text-zinc-500">Suppliers ({suppliers.length})</h2>
        <ul className="grid gap-2 sm:grid-cols-2">
          {suppliers.map((s) => (
            <li key={s.id} className="rounded-md border border-onyx bg-onyx px-4 py-2 text-sm">
              <span className="font-medium">{s.name}</span>
              <span className="block text-xs text-zinc-500">{[s.contact_name, s.email, s.phone].filter(Boolean).join(" · ")}</span>
            </li>
          ))}
          {suppliers.length === 0 ? <li className="text-sm text-zinc-500">No suppliers.</li> : null}
        </ul>
      </section>
    </main>
  );
